...
// Get the actual dimensions of the covered video
...
// Ensure canvas is always properly sized
...
function getKeyUnderPointer(x, y) {
  for (let key of keyboard) {
    ...
    // Add small tolerance for better usability
    const tolerance = 5;
    if (
...
function isHandOpen(landmarks) {
  // Returns true if all fingertips are above their base joints (open hand)
  // Thumb is ignored for simplicity
  return (
  ...
  switch (keyText) {
    ...
    case "↵":
      if (typedText.trim().length > 0) {
        // Create a blob with the typed text
      ...
      return ""; // Clear the input after download
    case "⌫":
...
function processHand(landmarks, handIndex) {
  const indexTip = landmarks[8];
  const handedness = indexTip.x < 0.5 ? "left" : "right"; // Determine which hand based on x position
  ...
  // Do NOT remove classes here; only do it once per frame in onResults
...
function onResults(results) {
  ...
  // Remove all hover/active classes from keys once per frame
  for (const key of keyboard) key.classList.remove("hovered", "glow");

  if (results.multiHandLandmarks.length > 0) {
    // Process each detected hand
    results.multiHandLandmarks.forEach((landmarks, index) => {
      // Only draw the index fingertip (landmark 8)
